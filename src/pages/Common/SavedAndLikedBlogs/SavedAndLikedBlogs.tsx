...
import { getUserLikedBlogs, getUserSavedBlogs, reportBlog, updateBlogLike, updateBlogSave } from '../../../services/Blogs';
    ...
    return (
        <div className="mx-auto  pb-10 dark:bg-dark lg:pb-20">
            ...
            {blogsData.map((blog, index) => {
                console.log(blog,'this si the lbog'); // Log the blog data
                return (
                    <BlogCard
                        key={index}
                        imageUrl={`${BASE_URL}${blog.blog.image}`} // Assuming 'imageUrl' is the correct property
                        date={formatDate(blog.blog.created_at)}
                        title={blog.blog.title}
                        author={blog.blog.user.full_name}
                        authorImage={blog.blog.user.profile?blog.blog.user.profile:"https://upload.wikimedia.org/wikipedia/commons/thumb/2/2c/Default_pfp.svg/2048px-Default_pfp.svg.png"}
                        calling={() => {
            ...
            {(page!=totalPages) && (<div className="flex items-center text-[10px] justify-center my-4">
            <button
                onClick={handlePrevPage}
                disabled={page === 1}
                className={`bg-slate-700 text-white py-2 px-4 rounded-full mx-2 transition duration-300 ease-in-out 
                            ${page === 1 ? 'opacity-50 cursor-not-allowed' : 'hover:bg-slate-800'}`}
            >
                Previous
            </button>
            ...
            <button
                ...
                className={`bg-slate-700 text-white py-2 px-4 rounded-full mx-2 transition duration-300 ease-in-out 
                            ${page === totalPages ? 'opacity-50 cursor-not-allowed' : 'hover:bg-slate-800'}`}
            >
                Next
            </button>
        </div>)}
